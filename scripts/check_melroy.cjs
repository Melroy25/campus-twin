const { Client, Databases, Query } = require('node-appwrite');

const client = new Client();
client
  .setEndpoint('https://nyc.cloud.appwrite.io/v1')
  .setProject('6a084d8900251e5c0f6e')
  .setKey(process.env.APPWRITE_API_KEY || 'standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function checkMelroy() {
  try {
    const students = await databases.listDocuments(DATABASE_ID, 'students', [Query.equal('email', '4so24cs128@campustwin.edu')]);
    console.log('Melroy Student:', students.documents[0]);
    if (students.documents[0]) {
      const uid = students.documents[0].uid;
      const roles = await databases.listDocuments(DATABASE_ID, 'userRoles', [Query.equal('uid', uid)]);
      console.log('Melroy userRole:', roles.documents[0]);
    }
  } catch (err) {
    console.error('Error:', err);
  }
}

checkMelroy();
