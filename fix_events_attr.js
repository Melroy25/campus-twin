import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    console.log("Adding missing attributes to events...");
    await databases.createStringAttribute(DATABASE_ID, 'events', 'image', 2000, false);
    console.log(" Added image");
    await new Promise(res => setTimeout(res, 500));
    
    await databases.createStringAttribute(DATABASE_ID, 'events', 'posted_by', 255, false);
    console.log(" Added posted_by");
    console.log("DONE");
  } catch (err) {
    console.error("Error:", err.message);
  }
}

run();
