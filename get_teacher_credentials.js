import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    console.log("Fetching teachers...");
    const teachersList = await databases.listDocuments(DATABASE_ID, 'teachers');
    console.log("Teachers in DB:");
    teachersList.documents.forEach(doc => {
      console.log(`- Name: ${doc.name}, Email: ${doc.email}, UID: ${doc.uid}`);
    });

    console.log("\nFetching user roles...");
    const rolesList = await databases.listDocuments(DATABASE_ID, 'userRoles');
    console.log("User Roles in DB:");
    rolesList.documents.forEach(doc => {
      console.log(`- Name: ${doc.name}, Role: ${doc.role}, USN: ${doc.usn || 'N/A'}, UID: ${doc.uid}`);
    });
  } catch (err) {
    console.error("Error:", err.message);
  }
}

run();
