import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  console.log("Adding category attribute to notifications...");
  try {
    await databases.createStringAttribute(DATABASE_ID, 'notifications', 'category', 255, false);
    console.log("Category attribute added successfully!");
  } catch (err) {
    if (err.code === 409) {
      console.log("Category attribute already exists.");
    } else {
      console.error("Error adding category attribute:", err.message);
    }
  }
}

run();
