import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  console.log("Adding 'feedback' attribute to 'placementCondoneRequests'...");
  try {
    await databases.createStringAttribute(DATABASE_ID, 'placementCondoneRequests', 'feedback', 1000, false);
    console.log("Success! Added 'feedback' attribute.");
  } catch (err) {
    if (err.code === 409) {
      console.log("Attribute already exists.");
    } else {
      console.error("Error creating attribute:", err.message);
    }
  }
}

run();
