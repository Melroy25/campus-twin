import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    const res = await databases.listDocuments(DATABASE_ID, 'placementCondoneRequests');
    console.log("=== CONDONE REQUESTS ===");
    res.documents.forEach(doc => {
      console.log(JSON.stringify(doc, null, 2));
    });
  } catch (err) {
    console.error("Error listing condone requests:", err);
  }
}

run();
