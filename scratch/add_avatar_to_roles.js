import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  console.log("Adding avatar_url to userRoles...");
  try {
    await databases.createStringAttribute(DATABASE_ID, 'userRoles', 'avatar_url', 2000, false);
    console.log("Successfully added avatar_url to userRoles");
  } catch (e) {
    console.error("Error adding to userRoles:", e.message);
  }
}

run();
