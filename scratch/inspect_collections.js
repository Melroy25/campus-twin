import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  const targetCols = ['complaints', 'leaveRequests', 'attendance', 'events', 'timetable', 'comments', 'class_messages', 'notifications'];
  for (const colName of targetCols) {
    try {
      const col = await databases.getCollection(DATABASE_ID, colName);
      console.log(`=== ${colName.toUpperCase()} COLLECTION SCHEMA ===`);
      console.log(col.attributes.map(a => `${a.key}: ${a.type} (${a.required ? 'required' : 'optional'})`).join(', '));
    } catch (e) {
      console.error(`Error loading ${colName}:`, e.message);
    }
  }
}

run();
