import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    console.log("Fetching subjectAllocations...");
    const allocations = await databases.listDocuments(DATABASE_ID, 'subjectAllocations');
    console.log("Allocations found:", allocations.total);
    for (const doc of allocations.documents) {
      console.log(`Document: ${doc.$id}, class_id: ${doc.class_id}, subject_id: ${doc.subject_id}, semester: ${doc.semester}`);
    }
  } catch (err) {
    console.error("Error fetching allocations:", err);
  }
}

run();
