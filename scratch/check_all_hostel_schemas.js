...
async function checkCollection(colName) {
  try {
    ...
    col.attributes.forEach(attr => {
      console.log(`- ${attr.key}: type=${attr.type}, size=${attr.size || 'N/A'}, required=${attr.required}, status=${attr.status}, array=${!!attr.array}`);
    });
  } catch (err) {
...
async function run() {
  ...
  await checkCollection('hostelRooms');
  await checkCollection('hostelComplaints');
}
