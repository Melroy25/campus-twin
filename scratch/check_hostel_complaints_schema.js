import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    const col = await databases.getCollection(DATABASE_ID, 'hostelComplaints');
    console.log("Collection name:", col.name);
    console.log("Attributes list:");
    col.attributes.forEach(attr => {
      console.log(`- ${attr.key}: type=${attr.type}, size=${attr.size || 'N/A'}, required=${attr.required}, status=${attr.status}`);
    });
  } catch (err) {
    console.error("Error fetching collection:", err.message);
  }
}

run();
