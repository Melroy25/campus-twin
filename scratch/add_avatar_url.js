...
async function run() {
  ...
  for (const col of collections) {
    console.log(`Adding avatar_url to ${col}...`);
    try {
      await databases.createStringAttribute(DATABASE_ID, col, 'avatar_url', 2000, false);
      console.log(`Successfully added avatar_url to ${col}`);
    } catch (e) {
      console.error(`Error adding to ${col}:`, e.message);
    }
  }
}
