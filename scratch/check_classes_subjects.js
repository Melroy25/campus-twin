import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    const response = await databases.listDocuments(DATABASE_ID, 'classes');
    console.log("=== CLASSES SUBJECT_IDS ===");
    response.documents.forEach(c => {
      console.log(`Class ID: ${c.$id}`);
      console.log(`Label: ${c.label}`);
      console.log(`subject_ids raw:`, c.subject_ids);
      try {
        const parsed = c.subject_ids ? JSON.parse(c.subject_ids) : [];
        console.log(`Parsed subject_ids:`, parsed);
      } catch (e) {
        console.log(`Parse error:`, e.message);
      }
      console.log('-------------------------------');
    });
  } catch (e) {
    console.error("ERROR:", e);
  }
}

run();
