import { Client, Databases, ID } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  const request = {
    complaint_id: `switch_${Date.now()}`,
    student_id: 'test_student_id',
    student_name: 'Test Student',
    student_usn: '4SO24CS128',
    hostel_type: 'boys',
    category: 'room_switch',
    title: `Room Switch Request: 101 → 201`,
    message: 'test switch reason',
    current_room: '101',
    requested_room: '201',
    status: 'pending',
    createdAt: new Date().toISOString()
  };

  try {
    const doc = await databases.createDocument(DATABASE_ID, 'hostelComplaints', ID.unique(), request);
    console.log("Successfully created doc:", doc.$id);
    // Delete it to keep DB clean
    await databases.deleteDocument(DATABASE_ID, 'hostelComplaints', doc.$id);
    console.log("Cleaned up successfully!");
  } catch (err) {
    console.error("Error creating doc:", err);
  }
}

run();
