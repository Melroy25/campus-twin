import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey(process.env.APPWRITE_API_KEY || 'standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';
const COLLECTION_ID = 'placementAnnouncements';

async function run() {
  console.log("Adding 'image_url' attribute to 'placementAnnouncements' collection...");
  try {
    await databases.createStringAttribute(DATABASE_ID, COLLECTION_ID, 'image_url', 2000, false);
    console.log("image_url attribute successfully added to placementAnnouncements!");
  } catch (err) {
    if (err.code === 409) {
      console.log("image_url attribute already exists.");
    } else {
      console.error("Error creating attribute:", err.message);
    }
  }
}

run();
