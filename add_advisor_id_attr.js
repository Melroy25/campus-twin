import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  try {
    console.log("Creating advisor_id attribute in classes collection...");
    await databases.createStringAttribute(DATABASE_ID, 'classes', 'advisor_id', 255, false);
    console.log("Attribute advisor_id created successfully!");
  } catch (e) {
    console.error("Error creating attribute:", e.message);
  }
}

run();
