import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('6a084d8900251e5c0f6e')
    .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = '6a084e9b00061aea385a';

async function run() {
  console.log("Adding image_url attribute to placementSessions collection...");
  try {
    await databases.createStringAttribute(DATABASE_ID, 'placementSessions', 'image_url', 2000, false);
    console.log("  Added image_url attribute!");
  } catch (err) {
    if (err.code === 409) {
      console.log("  image_url already exists.");
    } else {
      console.error("  Error:", err.message);
    }
  }
  console.log("Done!");
}

run();
